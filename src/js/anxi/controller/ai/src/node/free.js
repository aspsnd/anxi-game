...
export const FreeNode = new BehaviorTree(vita => {
    ...
    if (vita.aiController.haveSkill && vita.aiController.nextSkillTime < vita.timer) {
        for (const _skill of vita.aiController.hai.skill) {
            ...
            if (Math.random() > _skill.rate) continue;
            /**
             * @type {Vita}
             */
            let target;
    ...
    if (atkedEnemys.length > 0 && Math.random() * 20 <= 10 + vita.aiController.hai.intelli) {
        let target;
        if (vita.aiController.inDistance(vita.aiController.catchingEnemy,
    ...
    if (watchedEnemys.length > 0 && Math.random() > watchedEnemys[0].distance / 960) {
    ...
    if (Math.random() > 0.2) {
        Math.random() < 0.75 && (vita.face *= -1);
        let event = new ItemEvent('wantgo', parseInt(80 + 70 * Math.random()));
        let instruct = new Instruct(event);
        return instruct;
    }
